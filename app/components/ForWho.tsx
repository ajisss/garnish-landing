  ...
  return (
    <section id="for-who" className="py-28 bg-white">
      <div className="max-w-[1100px] mx-auto px-12">
        ...
        <div className="grid grid-cols-4 border border-zinc-100 rounded-2xl overflow-hidden">
          {personas.map((p, i) => {
            const Icon = p.icon;
            return (
              <div
                key={p.role}
                className={`flex flex-col items-center text-center p-8 bg-white ${i > 0 ? "border-l border-zinc-100" : ""}`}
              >
